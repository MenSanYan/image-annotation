function [U_final, V_final, nIter_final, objhistory_final] = GNMF_Multi_with_labelinf_for_multi_views_v2(X, k, Wr, Wi, options, U, V)
...
if epsion > 0
    ...
    Wm = epsion*Wm;
    DColm = full(sum(Wm,2));
    Dm = spdiags(DColm,0,nSmp,nSmp);
    Lm = Dm - Wm;
    if isfield(options,'NormW') && options.NormW
        D_mhalfr = spdiags(DColm.^-.5,0,nSmp,nSmp) ;
...
%all
Wm(1:numOftrain,1:numOftrain) = Wm(1:numOftrain,1:numOftrain) + Wi + Wr;
Dm(1:numOftrain,1:numOftrain) = Dm(1:numOftrain,1:numOftrain) + Di + Dr;
Lm(1:numOftrain,1:numOftrain) = Lm(1:numOftrain,1:numOftrain) + Lr - Li;

Wall = Wm;
Dall = Dm;
Lall = Lm;
Lall_n = (abs(Lall)-Lall)/2;
Lall_p = (abs(Lall)+Lall)/2;
...
if nRepeat == 1
    ...
    if isempty(maxIter)
        ...
        if isfield(options,'Converge') && options.Converge
            [objhistory,~,sacle] = CalculateObj(X, U, V, Lr, Li, options, coefficient);
        end
    end
else
    if isfield(options,'Converge') && options.Converge
...
%         %sacle
%         options.sacle = sacle/100;
%         Lall_n = options.sacle*Lall_n;
%         Lall_p = options.sacle*Lall_p;
        
        XUs = 0;
        VUUs = 0;
        for i=1:numOfviews
            UU = U{i}'*U{i};  % mk^2
            XUs = XUs+coefficient(i)*X{i}'*U{i};  % mnk or pk (p<<mn)
            VUUs = VUUs+coefficient(i)*V*UU; % nk^2
        end
        XUs = XUs + Lall_n*V;
        VUUs = VUUs + Lall_p*V;
        
        %update the Vl under the constrait of the semantic embedding.
        V = V.*(XUs./max(VUUs,1e-10));
...
%                 objhistory = CalculateObj(X, U, V, Lr, Li, options, coefficient)
        if nIter > minIter
            if selectInit
                ...
                if isempty(maxIter)
                    [newobj,~,sacle] = CalculateObj(X, U, V, Lr, Li, options, coefficient);
...
function [obj, dV,sacle] = CalculateObj(Xs, Us, V, Lr, Li, options, coefficient, deltaVU, dVordU)
    ...
    obj = obj_NMF+(obj_Lap_r-obj_Lap_i+obj_Lap_m)+obj_sparse_U+obj_sparse_V;%options.sacle*
    
    sacle = obj_NMF/(obj_Lap_r-obj_Lap_i+obj_Lap_m);
