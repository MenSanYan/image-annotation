function [U_final, V_final, nIter_final, objhistory_final] = GNMF_multi_views_v2(X, k, Wr, Wi, options, U, V)
...
if ~isfield(options,'coefficient')
    options.coefficient = ones(numOfviews,1);
end
...
if ~isfield(options,'epsion') || ~isfield(options,'Wm')
    options.epsion = 0;
end
...
switch lower(options.Optimization)
    case {lower('Multiplicative')} 
        [U_final, V_final, nIter_final, objhistory_final] = GNMF_Multi_with_labelinf_for_multi_views_v2(X, k, Wr, Wi, options, U, V);
    otherwise
